...
```
 ...
 구도 점검 (x,y = 화면 좌하단 0 ~ 우상단 1)
  CUT_05 F332  CAM_CUT05  | 주인공 (0.96,0.54) 보임  |  시민 (0.36,0.64) 보임
```

- `점검`: 1~720 프레임, 24fps, 마커 8개 위치, **모든 프레임에서 카메라가 올바른 컷 카메라인지** 확인
- `구도 점검`: 컷별 '이미지용 한 순간' 프레임에서 인물 머리·명찰의 화면 위치와 **세트에 가려졌는지(레이캐스트)** 확인
  → 카메라를 옮긴 뒤 인물이 가려지면 `[경고]` 가 뜹니다.
...
- 참고 속도(소프트웨어 GL, GPU 없음): Workbench 1280×720 약 0.7초/프레임(30초 ≈ 10분).
  GPU가 있는 PC의 Eevee는 이보다 훨씬 빠릅니다.
...
| 컷별 스틸 PNG | `renders/stills/CUT01_F0046.png` … |
| 저장된 씬 | `build/donghae_previs.blend` |
| 이 저장소에 첨부한 샘플 결과 | `preview/` (이 코드로 만든 Workbench MP4와 Eevee 스틸) |
